...
//  ]);
//  await db.ensureIndex('authors', name: 'meta', keys: {'_id': 1, 'name': 1, 'age': 1});
...
//  await collection.find(where.sortBy('age')).forEach((auth) => print("[${auth['name']}]:[${auth['email']}]:[${auth['age']}]"));
//  print('====================================================================');
//  print('>> Adding Users');
...
//  await usersCollection.find(where.sortBy('login', descending: true)).forEach((user) => print("[${user['login']}]:[${user['name']}]:[${user['email']}]"));
//  print('====================================================================');
//  print('>> Adding articles');
//  var articlesCollection = db.collection('articles');
//  await articlesCollection.insertAll([
//    {'title': 'Caminando por Buenos Aires', 'body': 'Las callecitas de Buenos Aires tienen ese no se que...', 'author_id': authors['Jorge Luis Borges']['_id']},
//    {
...
//    print("[${article['title']}]:[${article['body']}]:[${article['author_id'].toHexString()}]");
//  });
//  await db.close();
//}
